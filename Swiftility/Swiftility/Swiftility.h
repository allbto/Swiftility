//
//  Swiftility.h
//  Swiftility
//
//  Created by Allan Barbato on 12/14/15.
//  Copyright © 2015 Allan Barbato. All rights reserved.
//

#import <UIKit/UIKit.h>

//! Project version number for Swiftility.
FOUNDATION_EXPORT double SwiftilityVersionNumber;

//! Project version string for Swiftility.
FOUNDATION_EXPORT const unsigned char SwiftilityVersionString[];

// In this header, you should import all the public headers of your framework using statements like #import <Swiftility/PublicHeader.h>

#import <Swiftility/CALayer+Extensions.h>
#import <Swiftility/UIStoryboard+Extensions.h>
#import <Swiftility/UIView+Extensions.h>
#import <Swiftility/UIImageEffects.h>
